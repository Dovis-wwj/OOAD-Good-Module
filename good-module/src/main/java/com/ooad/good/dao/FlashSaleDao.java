...
public class FlashSaleDao {
     ...
     * @return
     */
    public ReturnObject<FlashSalePo> getFlashSaleById(Long flashId) {
        FlashSalePo po = null;
        try {
     ...
     * @return
     */
    public ReturnObject<List<FlashSaleItemPo>> selectItemsByFlashsaleId(Long id) {
        FlashSaleItemPoExample example = new FlashSaleItemPoExample();
        FlashSaleItemPoExample.Criteria criteria = example.createCriteria();
        criteria.andSaleIdEqualTo(id);
    ...
    public ReturnObject deleteFlashsaleItem(Long fid, Long id) {
        ...
        try {
            ...
            if (state.intValue() == 0 || state.intValue() == 2) {
                if (newPo.getFlashDate() == LocalDateTime.of(LocalDate.now(), LocalTime.MIN) ||//今天的秒杀
                        newPo.getFlashDate() == LocalDateTime.of(LocalDate.now().plusDays(1), LocalTime.MIN)) {//明天的秒杀
                    String key = "FlashSaleItem:" + newPo.getFlashDate().toString() + newPo.getTimeSegId().toString();
                    List<FlashSaleItemPo> flashSaleItemPos = selectItemsByFlashsaleId(id).getData();
    ...
    public ReturnObject modifyFlashSale(FlashSale flashSale) {
        ...
        try {
            ...
            //读出该条记录
            FlashSalePo  flashSalePo = flashSalePoMapper.selectByPrimaryKey(flashSale.getId());
            if (null == flashSalePo)//id没有对应数据
            {
                ...
                flashSalePo = flashSale.gotFlashSalePo();
     ...
     * @return
     */
    public ReturnObject<FlashSaleItemPo> getItemsByPrimaryKey(Long id) {
        try
        {
     ...
     * @return
     */
    public ReturnObject<FlashSalePo> selectByTimeAndId(LocalDateTime time, Long id) {
        FlashSalePoExample example = new FlashSalePoExample();
        FlashSalePoExample.Criteria criteria = example.createCriteria();
