...
/*
     ...
     * @return
     */
    @ApiOperation(value = "新增店铺", produces = "application/json;charset=UTF-8")
    @ApiImplicitParams({
...
//        if(id!=departId&&departId!=0)
  //          return Common.decorateReturnObject(new ReturnObject<>(ResponseCode.RESOURCE_ID_OUTSCOPE));
        ...
        else if(returnObj.getCode()==ResponseCode.FIELD_NOTVALID)
        {
            httpServletResponse.setStatus(HttpStatus.BAD_REQUEST.value());
            return Common.getNullRetObj(returnObj, httpServletResponse);
        }
        else
    ...
    @ResponseBody
    public Object deleteShop(@PathVariable Long id,@Depart Long departId){
    ...
    public Object auditShopInfo(@PathVariable Long shopId, @PathVariable Long id, @Validated @RequestBody ShopConVo vo, BindingResult bindingResult)
    {
        if (logger.isDebugEnabled()) {
            logger.debug("judge ShopInfo: id = "+ shopId +" vo = " + vo);
        }
        // 校验前端数据
        Object returnObject = Common.processFieldErrors(bindingResult, httpServletResponse);
        if (returnObject != null) {
        ...
        ReturnObject retObject = shopService.auditShopInfo(id,vo.getConclusion());
